...
void GameController::draw(sf::RenderWindow &window) {
	...
	if (this->getIsGameOver()) {
		...
		tC.setString("You Win");
		tC.setFont(font);
		tC.setCharacterSize(60);
		tC.setFillColor(this->map[0].getRect().getOutlineColor());
		tC.setPosition(730, 400);
		window.draw(tC);

		sf::Text space;

		space.setString("click space to play again");
		space.setFont(font);
		space.setCharacterSize(20);
		space.setFillColor(sf::Color::Black);
		space.setPosition(730, 700);
		window.draw(space);
		
	}
	if (this->getIsGameOver() && sf::Keyboard::isKeyPressed(sf::Keyboard::Space)) {
		this->reset();
	}
	...
	//licznik tur
	auto strTC = std::to_string(5 - this->tourCount);
	sf::Text tC;

	tC.setString(strTC);
	tC.setFont(font);
	tC.setCharacterSize(30);
	tC.setFillColor(tour);
	tC.setPosition(1, 1);
	window.draw(tC);


	//wybrana liczba armii
...
void GameController::play(sf::RenderWindow & window) {
	...
	sf::Color buffer = this->map[0].getRect().getOutlineColor();
	for (auto i = 0; i < 29; i++) {
		if (this->map[i].getRect().getOutlineColor() == buffer) {
			if (i == 28) this->gameOver = true;
			else continue;
		}
		else break;
...
bool GameController::getIsGameOver()
{
...
void GameController::reset()
{
	...
	this->from = -1;
	this->to = -1;
	this->tour = sf::Color::Blue;
	this->army = 1000;//armia z suwaka
	this->tourCount = 2;
	this->gameOver = false;
}
